...
describe('BillingAddressDetailPage', () => {
  ...
  let billingAddressClientMock: { getBillingAddress: Mock; updateBillingAddress: Mock };
  ...
  describe('checkPermissions', () => {
    ...
    it('should set NoPermission status when the user lacks billing permission', async () => {
      ...
      expect(billingAddressClientMock.getBillingAddress).not.toHaveBeenCalled();
    });
    ...
    it('should proceed to load the billing address when permission is granted', async () => {
      ...
      fixture.detectChanges();
      await vi.waitFor(() => expect(billingAddressClientMock.getBillingAddress).toHaveBeenCalled());

      expect(authMock.waitForAuth).toHaveBeenCalled();
      expect(billingAddressClientMock.getBillingAddress).toHaveBeenCalledWith({
        subscriptionID: SUBSCRIPTION_ID,
      });
    });
  });
  ...
  describe('onSubmit', () => {
    ...
    it('should not call updateBillingAddress when the form is invalid', () => {
      ...
      void component.onSubmit();
      expect(billingAddressClientMock.updateBillingAddress).not.toHaveBeenCalled();
    });
    ...
    it('should submit the mapped address and show a success snackbar on success', async () => {
      ...
      expect(billingAddressClientMock.updateBillingAddress).toHaveBeenCalledWith({
        subscriptionID: SUBSCRIPTION_ID,
        name: 'John Smith',
        phone: '+49 000 000000',
        address: {
    ...
    it('should show an error snackbar when the update reply has no subscriptionID', async () => {
      billingAddressClientMock.updateBillingAddress.mockReturnValue(of({ subscriptionID: '' }));
