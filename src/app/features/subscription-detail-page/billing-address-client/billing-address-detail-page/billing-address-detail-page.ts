...
@Component({
  selector: 'app-billing-address-detail-page',
  templateUrl: './billing-address-detail-page.html',
  ...
  styleUrl: './billing-address-detail-page.scss',
  imports: [
    MatCard,
    MatProgressBar,
    MatCardTitle,
...
export class BillingAddressDetailPage implements OnInit {
  ...
  private readonly billingAddress = inject(BillingAddressClient);
  ...
  readonly form: FormGroup<BillingAddressFormControls> = this.formBuilder.nonNullable.group({
    ...
    phone: ['', [Validators.required, Validators.pattern(BillingAddressDetailPage.PHONE_PATTERN)]],
  });
  ...
  async checkPermissions() {
    ...
    if (!subscriptionID) {
      this.log.error('BillingAddressDetailComponent.checkPermissions: subscriptionID is null');
  ...
  async reloadBillingAddressDetail(): Promise<void> {
    ...
    const subscriptionID = this.getSubscriptionID();
    if (!subscriptionID) {
      this.log.error('BillingAddressDetailComponent.reloadBillingAddressDetail: subscriptionID is null');
      return;
    }
