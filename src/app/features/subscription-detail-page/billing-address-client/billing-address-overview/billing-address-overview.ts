...
@Component({
  ...
  templateUrl: './billing-address-overview.html',
  styleUrl: './billing-address-overview.scss',
...
export class BillingAddressOverview implements OnInit {
  ...
  private readonly billingAddressService = inject(BillingAddressClient);
  private readonly route = inject(ActivatedRoute);
  private readonly log = inject(Log);
  private readonly snackBar = inject(SnackBar);
  private readonly returnUrl = inject(ReturnUrl);
  private readonly countryService = inject(CountryLookup);
  private readonly languageService = inject(LanguageStore);
  ...
  private async checkPermissions(): Promise<void> {
    ...
    try {
      const hasPermission = await this.permissionStore.isBilling(id);
      if (hasPermission) {
