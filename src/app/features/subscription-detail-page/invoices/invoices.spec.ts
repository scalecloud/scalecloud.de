...
describe('Invoices', () => {
  ...
  let invoicesClientMock: { getInvoices: ReturnType<typeof vi.fn> };
  let authMock: { waitForAuth: ReturnType<typeof vi.fn> };
  let logMock: { error: ReturnType<typeof vi.fn> };
  let permissionStoreMock: { isBilling: ReturnType<typeof vi.fn> };
  ...
  function configureTestBed(paramMapValue: Record<string, string> = { subscriptionID }) {
    ...
    invoicesClientMock = { getInvoices: vi.fn().mockReturnValue(of(buildReply())) };
    authMock = { waitForAuth: vi.fn().mockResolvedValue(undefined) };
    logMock = { error: vi.fn() };
    permissionStoreMock = { isBilling: vi.fn().mockResolvedValue(true) };
    snackBarMock = { error: vi.fn(), infoDuration: vi.fn() };

    TestBed.configureTestingModule({
      imports: [Invoices],
      providers: [
        { provide: InvoicesClient, useValue: invoicesClientMock },
  ...
  describe('permission checks', () => {
    it('should set status to Error and log when subscriptionID is missing', async () => {
      ...
      expect(permissionStoreMock.isBilling).not.toHaveBeenCalled();
    });
    ...
    it('should load invoices when the user has billing permission', async () => {
      ...
      expect(permissionStoreMock.isBilling).toHaveBeenCalledWith(subscriptionID);
    ...
    it('should set status to NoPermission when the user lacks billing permission', async () => {
      ...
      expect(invoicesClientMock.getInvoices).not.toHaveBeenCalled();
    });

    it('should set status to Error and show a snackbar when the permission check throws', async () => {
      configureTestBed();
      permissionStoreMock.isBilling.mockRejectedValue(new Error('boom'));
      await fixture.whenStable();
  ...
  describe('getInvoices', () => {
    ...
    it('should set status to Loading while the request is in flight', async () => {
      ...
      invoicesClientMock.getInvoices.mockReturnValue(subject.asObservable());
    ...
    it('should populate reply and set status to Success on a successful response', async () => {
      ...
      invoicesClientMock.getInvoices.mockReturnValue(of(reply));
      await fixture.whenStable();
    ...
    it('should set status to Error when the request fails', async () => {
      invoicesClientMock.getInvoices.mockReturnValue(throwError(() => new Error('network error')));
      await fixture.whenStable();
    ...
    it('should pass pageSize and subscriptionID in the request', async () => {
      ...
      expect(invoicesClientMock.getInvoices).toHaveBeenCalledWith(
  ...
  describe('pagination', () => {
    beforeEach(async () => {
      configureTestBed();
      await fixture.whenStable();
      invoicesClientMock.getInvoices.mockClear();
    });
