...
describe('Seats', () => {
  ...
  beforeEach(async () => {
    mocks = buildMocks();
    mocks.seatsClient.getListSeats.mockReturnValue(of(makeReply()));

    await TestBed.configureTestingModule({
      imports: [Seats],
      providers: [
        { provide: SeatsClient, useValue: mocks.seatsClient },
        ...
        { provide: PermissionStore, useValue: mocks.permissionStore },
  ...
  it('sets NoPermission when user is not an administrator', async () => {
    mocks.permissionStore.isAdministrator.mockResolvedValue(false);
    await component.checkPermissions();
  ...
  it('sets Error and shows snackbar when permission check throws', async () => {
    mocks.permissionStore.isAdministrator.mockRejectedValue(new Error('network'));
    await component.checkPermissions();
    expect(component.serviceStatus()).toBe(ServiceStatus.Error);
  ...
  it('transitions Loading → Error when API fails', async () => {
    mocks.seatsClient.getListSeats.mockReturnValue(throwError(() => new Error('500')));
    component.loadSeats();
    await fixture.whenStable();
    expect(component.serviceStatus()).toBe(ServiceStatus.Error);
  });
  ...
  it('usedSeats returns totalResults', async () => {
    mocks.seatsClient.getListSeats.mockReturnValue(of(makeReply({ totalResults: 3 })));
    component.loadSeats();
    await fixture.whenStable();
  ...
  it('maxSeats returns maxSeats from reply', async () => {
    mocks.seatsClient.getListSeats.mockReturnValue(of(makeReply({ maxSeats: 10 })));
    component.loadSeats();
    await fixture.whenStable();
  ...
  it('isAddSeatPossible is false when seats === maxSeats', async () => {
    ...
    mocks.seatsClient.getListSeats.mockReturnValue(of(makeReply({ maxSeats: 5, seats, totalResults: 5 })));
    component.loadSeats();
    await fixture.whenStable();
